...
Required (configure in repo Settings > Secrets > Actions):
- `OPENAI_API_KEY`
...
- Slack/Discord notifications (add secret + step)
- Auto-cancel older runs (`cancel-in-progress: true`)
- Matrix for multiple clip durations

README snippet (for inline insertion):

```markdown
### Automation Workflows

Workflows:
1. `autopost.yml` (gated) — scheduled hourly, executes only at defined times unless `force=true`.
2. `agent-trigger.yml` (direct) — runs immediately on dispatch.

Dispatch examples:

# Gated run (may skip if not time)
