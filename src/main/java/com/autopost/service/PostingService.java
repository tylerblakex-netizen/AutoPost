...
public class PostingService {
  ...
  private boolean hasTwitterCredentials() {
    return twitterApiKey != null
        && !twitterApiKey.trim().isEmpty()
        && twitterApiSecret != null
        && !twitterApiSecret.trim().isEmpty()
        && twitterAccessToken != null
        && !twitterAccessToken.trim().isEmpty()
        && twitterAccessSecret != null
        && !twitterAccessSecret.trim().isEmpty();
  }

  private boolean hasGoogleCredentials() {
    return serviceAccountPath != null
        && !serviceAccountPath.trim().isEmpty()
        && rawFolderId != null
        && !rawFolderId.trim().isEmpty()
        && editsFolderId != null
        && !editsFolderId.trim().isEmpty()
  ...
  public void checkAndPost() throws Exception {
    ...
    Map<String, Object> nextRun =
        new com.fasterxml.jackson.databind.ObjectMapper().readValue(content, Map.class);
  ...
  private void logPostSuccess(LocalDate date, String caption) throws IOException {
    ...
    String logEntry =
        String.format(
            "%s - Posted successfully. Caption: %s%n",
