...
public class Runner {
  ...
  private static final Map<String, Integer> DOW =
  ...
  private boolean inBestSlot() {
    ...
    try {
      ...
      int d = now.getDayOfWeek().getValue() - 1;
      int h = now.getHour();
      for (var s : root.get("slots"))
        if (DOW.get(s.get("day").asText()) == d && s.get("hour").asInt() == h) return true;
      return false;
    } catch (Exception e) {
      return true;
    }
  ...
  public void run() throws Exception {
    if (!inBestSlot()) {
      log.info("Not in best posting slot now. Skipping.");
      return;
    }
    ...
    if (f == null) {
      log.info("No files in RAW. Exiting.");
      return;
    }
